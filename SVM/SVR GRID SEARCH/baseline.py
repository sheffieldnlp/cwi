...
class Baseline(object):
    ...
    def __init__(self, language):
        ...
        #self.model = svm.SVR(C=100, cache_size=200, class_weight=None, coef0=0.0,
    ...
    def extract_features(self, word,sent):
        ...
        return [str(round(len_chars,4)), str(len_tokens),str(senses),str(sylable)]
    ...
    def test(self, testset):
        ...
        for sent in testset:
            X.append(self.extract_features(sent['target_word'],sent['sentence']))
        #df1=pd.DataFrame(X,columns=['len_chars','len_tokens','senses','syllabules'])
        a = self.clf.predict(np.array(X))
        print(a)
        print(a.round())
        return a.round() #self.model.predict(df1)
