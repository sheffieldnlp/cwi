...
class Baseline(object):
    ...
    def __init__(self, language):
       ...
       # self.clf = GridSearchCV(svm.SVR(), self.parameters)
        self.rnd_search= RandomizedSearchCV(AdaBoostRegressor(random_state=42),param_distributions = self.param_dist,cv=3,n_iter = 10,n_jobs=-1,random_state=42)
    ...
    def POS_tag(self,word,sent):
        ...
        if self.language == 'english':
            ...
            doc = nlp(sent)  
            for token in doc:
                temp2.append(token.ent_iob_)
                int_encoded = self.Label_Encoder_BIOS.transform(pd.DataFrame(temp2,columns=['BIOS_tags']).values.ravel())
                one_hot_encoded = self.OneHot_BIOS.transform(np.array(pd.DataFrame(int_encoded,columns=['BIOS'])).reshape(-1,1)).toarray()
                one_hot_encoded_str2 = [str(i) for i in list(one_hot_encoded[0])]
                temp2=[]
                X[token.text] = [one_hot_encoded_str2,str(int(token.is_alpha==True)), str(int(token.is_stop==True)),str(int(token.is_ascii==True)),str(token.sentiment),str(0)] # Reduced Feature Vector
            if word not in X:
                doc = nlp(word)
                for token in doc.noun_chunks:
                    temp2.append(token.root.ent_iob_)
                    int_encoded = self.Label_Encoder_BIOS.transform(pd.DataFrame(temp2,columns=['BIOS_tags']).values.ravel())
                    one_hot_encoded = self.OneHot_BIOS.transform(np.array(pd.DataFrame(int_encoded,columns=['BIOS'])).reshape(-1,1)).toarray()
                    one_hot_encoded_str2 = [str(i) for i in list(one_hot_encoded[0])]
                    temp2=[]
                    X[word] = [one_hot_encoded_str2,str(int(token.is_alpha==True)), str(int(token.is_stop==True)),str(int(token.is_ascii==True)),str(token.sentiment),str(0)] # Reduced Feature Vector                               
        else:
            ...
            if word not in X:
                ...
                for token in doc:     
                    ...
                    temp=[]
                    temp2=[]
                    X[word] = [one_hot_encoded_str2,str(int(token.is_alpha==True)), str(int(token.is_stop==True)),str(int(token.is_ascii==True)),str(token.sentiment),str(1)]#is a noun phrase
        return X
    ...
    def test(self, testset):
        ...
        return self.rnd_search.predict(X).round() #self.clf.predict(X).round() #self.rnd_search.predict(X).round() #self.model.predict(np.array(X,object)) 
